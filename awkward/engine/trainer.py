...
class MyEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        x, x2, y = x.to(engine.device), x2.to(engine.device), y.to(engine.device)
        
        x = x[:, :engine.config.max_length]
        x2 = x2[:, :engine.config.max_length]
        
        x = pad_to_maxseq_to_batch(x, engine.config.max_length, engine.device)
        x2 = pad_to_maxseq_to_batch(x2, engine.config.max_length, engine.device)
        ...
        #print("y_hat : ", y_hat)
        print("y : ", y)
        print("argmax : ", torch.argmax(y_hat, dim=-1))
    ...
    def validate(engine, mini_batch):
        ...
        with torch.no_grad():
##             x, y = mini_batch.text1, mini_batch.label
##             x, y = x.to(engine.device), y.to(engine.device)
    ...
    def save_model(engine, train_engine, config, **kwargs):
        ...
        #torch.jit.save(engine.best_model, 'model_jit.pt') # jit으로 저장해야 jit으로 읽을 수 있다.
